...
%%
  ...
  %% Calculate shape functions            
    ...
    BG = repmat([repmat([1 0 0],3, 8);repmat([0 1 0],3, 8);repmat([0 0 1],3, 8)].*repmat(temp, 3,1,1,1), 1, 1, NE, 1);
    ...
    d0U = permute(reshape(permute(reshape(d0U, [3 1 3 8 NE]), [1 3 2 4 5]), [3 3 8 NE]), [1 2 4 3]);
    d0U_int = pagemtimes(GDSP_int, 'none', repmat(SHPD, 1, 1, NE), 'transpose');
    d0U_int = permute(reshape(permute(reshape(d0U_int, [3 1 3 8 NE]), [1 3 2 4 5]), [3 3 8 NE]), [1 2 4 3]);
    ...
    BN = [temp.*repmat(FT_int, 1, 8, 1, 1); temp.*repmat(circshift(FT_int,[-1 0 0 0]), 1, 8, 1, 1) + circshift(temp,[-1 0 0 0]).*repmat(FT_int, 1, 8, 1, 1)];
        %% Compute stress
    ...
    gplane_stress = repmat(plane_stress, 1, 1, NE, 8);
    % second piola kirchhoff stress tensor (nonlinear)
    nintSTRESS = (lambda.*sum(E_int.*tr, [1 2]) .* tr + 2*mu.*E_int) .* gplane_stress;
    lSTRESS = (lambda.*sum(epsilon.*tr, [1 2]) .* tr + 2*mu.*epsilon) .* gplane_stress; 
    lintSTRESS = (lambda.*sum(epsilon_int.*tr, [1 2]) .* tr + 2*mu.*epsilon_int) .* gplane_stress; 
    ...
    gstiff = DET*sum(pagemtimes(pagemtimes(BN, 'transpose', repmat(ETAN,1,1,1,8), 'none'),BN) + pagemtimes(pagemtimes(BG, 'transpose', SHEAD, 'none'),BG),4);
