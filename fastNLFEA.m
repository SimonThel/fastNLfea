function U = fastNLFEA(ITRA, TOL, ATOL, NTOL, TIMS, nu, E0, Emin, penal, xPhys, ...
    EXTFORCE, SDISPT, XYZ, LE)
%******************************************************************** 
...
GDOF = int32(reshape ([1 + (LE-1)*NDOF; 2 + (LE-1)*NDOF; 3 + (LE-1)*NDOF],NE, 24));
iK = int32(reshape(repmat(GDOF, 1,24)', 576*NE, 1));
jK = int32(reshape(repmat(reshape(GDOF',1, 24*NE), 24, 1), 1 , 576*NE)');
lK = int32(reshape(permute(GDOF,[2 1 3 4]), 24*NE, 1));
FIXEDDOF=int32(NDOF*(SDISPT(:,1)-1)+SDISPT(:,2)); 
ALLDOF=1:NEQ;
...
while(FLAG10 == 1)                      % Solution has been converged
    ...
    while(FLAG11 == 1)                   % Bisection loop start
        ...
        while(FLAG20 == 1)
            ...
            % Assemble K and F
            [GKF, FORCE] = assembly(DISPTD, xPhys, penal, beta, eta, NE, NEQ, NDOF, LE, lambda, mu, ETAN, DET, BG, gSHPDT, temp, iK, jK, lK);
            % Increase external force
            if size(EXTFORCE,1)>0
                LOC = NDOF*(EXTFORCE(:,1)-1)+EXTFORCE(:,2);
                FORCE = FORCE + fsparse(int32(LOC),1,double(FACTOR*EXTFORCE(:,3)), [double(NEQ), 1]);
            end
            % Check convergence
            if(ITER>1)
